...
app_status_t process_common_convert_to_yuv420(process_common_ctx_t *ctx, video_frame_t *frame) {
    ...
    if (ctx->yuv420_convert.dst_width != frame->av_frame->width ||
        ...
        video_frame_convert_deinit(&ctx->yuv420_convert);
        if (video_frame_convert_init(&ctx->yuv420_convert,
    ...
    converted_frame = video_frame_convert_prepare(&ctx->yuv420_convert, frame->av_frame, &converted);
    ...
    if (converted) {
        ...
        LOGT("process convert frame source=%s -> yuv420p size=%dx%d",
             frame->source_name,
             frame->av_frame->width,
             frame->av_frame->height);
    }
