...
app_status_t vp_init(vp_ctx_t **out_ctx, const vp_config_t *cfg) {
    ...
    for (i = 0; i < p->channel_count; ++i) {
        ...
        ch->capture_reconnect =
            ch->input.source_type == VIDEO_SOURCE_HDMI_IN ||
            ch->input.source_type == VIDEO_SOURCE_USB;
        ...
        if (status != APP_OK) {
            if (ch->input.source_type == VIDEO_SOURCE_USB) {
                LOGW("%s input open failed: %s (will retry until USB device is available)",
                     ch->name,
                     app_status_str(status));
            } else if (ch->input.source_type == VIDEO_SOURCE_HDMI_IN &&
                status == APP_ERR_IO) {
                LOGW("%s input open failed: %s (will retry until HDMI signal is available)",
                     ch->name,
                     app_status_str(status));
            } else if (ch->input.source_type == VIDEO_SOURCE_HDMI_IN) {
                ch->capture_reconnect = 0;
                LOGE("%s input open failed: %s (channel disabled)",
                     ch->name,
                     app_status_str(status));
                ch->is_opened = 0;
                continue;
        ...
        if (ch->is_opened) {
            ...
            LOGI("%s capture thread started in reconnect mode: waiting for input",
                 ch->name);
        }
    }
    ...
    if (stream_output != VP_STREAM_OUTPUT_UDP &&
        stream_output != VP_STREAM_OUTPUT_RTSP &&
        stream_output != VP_STREAM_OUTPUT_SRT) {
        stream_output = VP_STREAM_OUTPUT_RTSP;
    }
    ...
    status = video_frame_convert_init_hw(&p->stream_convert,
                                           stream_w,
                                           stream_h,
                                           AV_PIX_FMT_NV12);
    if (status != APP_OK) {
    ...
    status = osd_rkrga_init(&p->stream_osd_rkrga,
                                   stream_w,
                                   stream_h,
                                   stream_w,
                                   stream_h,
    ...
    p->stream_osd_rgba->width = stream_w;
    p->stream_osd_rgba->height = stream_h;
    ...
    if (p->cfg.source_label.enabled) {
        ...
        status = osd_rkrga_init(&p->stream_label_rkrga,
                                       stream_w,
                                       stream_h,
                                       label->width,
                                       label->height,
    ...
    udp_destination_ip = cfg->stream_output.udp_dest_ip[0] != '\0'
                             ? cfg->stream_output.udp_dest_ip
                             : cfg->stream_output.rtp_dest_ip;
    udp_destination_port = cfg->stream_output.udp_dest_port > 0
                               ? cfg->stream_output.udp_dest_port
                               : cfg->stream_output.rtp_dest_port;
    if (stream_output == VP_STREAM_OUTPUT_UDP &&
        udp_destination_port > 0 && udp_destination_ip[0] != '\0') {
        LOGI("stream output selected: UDP/RTP -> %s:%d",
             udp_destination_ip,
             udp_destination_port);
        if (udp_output_init(&p->udp_output,
                            udp_destination_ip,
                            udp_destination_port) != APP_OK) {
            LOGW("udp open failed, streaming disabled");
            p->stream_enabled = 0;
        }
    } else if (stream_output == VP_STREAM_OUTPUT_UDP) {
        LOGW("stream dest not configured, streaming disabled");
        p->stream_enabled = 0;
    } else if (stream_output == VP_STREAM_OUTPUT_SRT) {
        LOGI("stream output selected: SRT -> %s",
             p->cfg.stream_output.srt_url[0] != '\0'
                 ? p->cfg.stream_output.srt_url
                 : VP_DEFAULT_SRT_URL);
        LOGD("SRT publish will open after the first hardware OSD frame");
    } else {
        LOGI("stream output selected: RTSP/%s -> %s",
             p->cfg.stream_output.rtsp_transport[0] != '\0'
                 ? p->cfg.stream_output.rtsp_transport
                 : VP_DEFAULT_RTSP_TRANSPORT,
             p->cfg.stream_output.rtsp_url[0] != '\0'
                 ? p->cfg.stream_output.rtsp_url
                 : VP_DEFAULT_RTSP_URL);
        LOGD("RTSP publish will open after the first hardware OSD frame");
    }
    ...
    LOGD("stream queue mode=latest-frame (stale frames dropped)");
...
app_status_t vp_snapshot_ex(vp_ctx_t *p,
    ...
    if (ch->input.source_type == VIDEO_SOURCE_HDMI_IN) {
        hdmi_has_signal = ch->input_has_signal;
    }
    pthread_mutex_unlock(&p->lock);
    if (!hdmi_has_signal) {
        LOGW("%s snapshot skipped: HDMI has no valid input signal", ch->name);
        return APP_ERR_IO;
    }
    ...
    status = mp4_recorder_write_jpeg(frame, path);
    av_frame_free(&frame);
...
void vp_get_channel_stats(vp_ctx_t *p, vp_channel_id_t id, vp_channel_stats_t *out) {
    ...
    out->capture_cpu_ms = p->ch[id].capture_cpu_ms;
    out->capture_cpu_load = p->ch[id].capture_cpu_load;
    out->read_ms = p->ch[id].read_ms;
    out->decode_ms = p->ch[id].decode_ms;
    out->record_encode_ms = p->ch[id].record_encode_ms;
    out->record_queue = p->ch[id].record_queue;
    out->record_queue_peak = p->ch[id].record_queue_peak;
    out->record_dropped = p->ch[id].record_dropped;
    pthread_mutex_unlock(&p->lock);
}

void vp_get_stream_stats(vp_ctx_t *p, vp_stream_stats_t *out) {
    if (p == NULL || out == NULL) {
        return;
    }
    pthread_mutex_lock(&p->lock);
...
void vp_deinit(vp_ctx_t *p) {
    ...
    /* 先停采集线程：stop 已置位，采集线程退出后不再有新帧入队 */
    for (i = 0; i < p->channel_count; ++i) {
        vp_channel_t *ch = &p->ch[i];
    ...
    /* 再停录制线程：此时没有新帧，录制线程可干净退出 */
    vp_record_stop_all(p);
    ...
    osd_rkrga_deinit(&p->stream_osd_rkrga);
    if (p->stream_osd_rgba != NULL) {
        av_frame_free(&p->stream_osd_rgba);
    }
    osd_rkrga_deinit(&p->stream_label_rkrga);
    if (p->stream_label_rgba != NULL) {
        av_frame_free(&p->stream_label_rgba);
    }
    video_frame_convert_deinit(&p->stream_convert);
