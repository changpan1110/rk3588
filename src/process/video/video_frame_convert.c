...
#define LOG_FILE_NAME "video_frame_convert.c"
...
#ifdef HAVE_LIBRGA
static pthread_mutex_t g_video_frame_convert_rga2_lock = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t g_video_frame_convert_rga3_lock = PTHREAD_MUTEX_INITIALIZER;

static pthread_mutex_t *video_frame_convert_rga_lock_for_core(IM_SCHEDULER_CORE core) {
    if (core == IM_SCHEDULER_RGA2_CORE0 || core == IM_SCHEDULER_RGA2_CORE1) {
        return &g_video_frame_convert_rga2_lock;
    }
    return &g_video_frame_convert_rga3_lock;
}

static IM_STATUS video_frame_convert_rga_resize_on_core(rga_buffer_t src,
                                                  rga_buffer_t dst,
    ...
    {
        pthread_mutex_t *lock = video_frame_convert_rga_lock_for_core(core);
...
static IM_STATUS video_frame_convert_rga_cvtcolor_on_core(rga_buffer_t src,
                                                    rga_buffer_t dst,
                                                    int src_fmt,
                                                    int dst_fmt,
...
static int video_frame_convert_sws_colorspace(const AVFrame *frame) {
...
static int video_frame_convert_frame_is_rgb(const AVFrame *frame) {
...
static int video_frame_convert_format_is_yuvj(enum AVPixelFormat fmt) {
...
static enum AVPixelFormat video_frame_convert_sws_format(enum AVPixelFormat fmt) {
    switch (fmt) {
...
static int video_frame_convert_dst_is_limited_bt709(const video_frame_convert_ctx_t *ctx) {
    return ctx->dst_fmt == AV_PIX_FMT_NV12 ||
           ctx->dst_fmt == AV_PIX_FMT_NV21 ||
           ctx->dst_fmt == AV_PIX_FMT_NV16;
}

static int video_frame_convert_frame_is_hw(const AVFrame *frame) {
...
static const char *video_frame_convert_pix_fmt_name(enum AVPixelFormat fmt) {
    const char *name = av_get_pix_fmt_name(fmt);

    return name != NULL ? name : "unknown";
}

static void video_frame_convert_reset_rkrga_filter(video_frame_convert_ctx_t *ctx) {
    video_rkrga_filter_destroy(&ctx->rkrga_filter);
...
static int video_frame_convert_rkrga_source_matches(const video_frame_convert_ctx_t *ctx,
...
static int video_frame_convert_remember_rkrga_source(video_frame_convert_ctx_t *ctx,
                                                const AVFrame *src_frame,
                                                int input_is_hw) {
    av_buffer_unref(&ctx->rkrga_src_hw_frames_ctx);
...
static const AVFrame *video_frame_convert_transfer_hw_frame(video_frame_convert_ctx_t *ctx,
                                                       const AVFrame *src_frame) {
    AVHWFramesContext *frames_ctx;
    int ret;

    if (!video_frame_convert_frame_is_hw(src_frame)) {
        return src_frame;
    }
    ...
    if (!ctx->hw_transfer_path_logged) {
        ...
        LOGW("encode fallback path=hardware-download src=%s sw_fmt=%s",
             video_frame_convert_pix_fmt_name((enum AVPixelFormat)src_frame->format),
             frames_ctx != NULL
                 ? video_frame_convert_pix_fmt_name(frames_ctx->sw_format)
                 : "unknown");
...
static const AVFrame *video_rkrga_filter_convert(
    video_frame_convert_ctx_t *ctx,
    const AVFrame *src_frame,
    int *converted) {
    ...
    input_is_hw = video_frame_convert_frame_is_hw(src_frame);
    if (ctx->dst_fmt != AV_PIX_FMT_NV12) {
        return NULL;
    }

    if (ctx->rkrga_filter_failed) {
        if (video_frame_convert_rkrga_source_matches(ctx, src_frame, input_is_hw)) {
            return NULL;
        }
        video_frame_convert_reset_rkrga_filter(ctx);
        ctx->rkrga_filter_failed = 0;
        ctx->rkrga_filter_path_logged = 0;
    } else if (ctx->rkrga_filter != NULL &&
               !video_frame_convert_rkrga_source_matches(ctx, src_frame, input_is_hw)) {
        video_frame_convert_reset_rkrga_filter(ctx);
        ctx->rkrga_filter_path_logged = 0;
    }
    if (ctx->rkrga_filter == NULL) {
        if (!video_frame_convert_remember_rkrga_source(ctx, src_frame, input_is_hw)) {
            return NULL;
        }
        if (input_is_hw) {
            status = video_rkrga_filter_create_hw(&ctx->rkrga_filter,
                                                          src_frame->width,
                                                          src_frame->height,
            ...
            status = video_rkrga_filter_create(&ctx->rkrga_filter,
                                               src_frame->width,
                                               src_frame->height,
    ...
    output_frame = video_rkrga_filter_process(ctx->rkrga_filter, src_frame);
    ...
    if (!ctx->rkrga_filter_path_logged) {
        LOGD("encode convert path=%s src=%dx%d fmt=%s "
             "dst=%dx%d fmt=%s",
             input_is_hw ? "FFmpeg-RKRGA-hardware-frame"
                         : "FFmpeg-software-frame-hwupload",
             src_frame->width,
             src_frame->height,
             video_frame_convert_pix_fmt_name((enum AVPixelFormat)src_frame->format),
             ctx->dst_width,
             ...
             ctx->output_is_hw ? "drm_prime(nv12)"
                               : video_frame_convert_pix_fmt_name(ctx->dst_fmt));
...
static int video_frame_convert_configure_sws(video_frame_convert_ctx_t *ctx,
                                       const AVFrame *src_frame) {
    ...
    if (!video_frame_convert_dst_is_limited_bt709(ctx) &&
        !video_frame_convert_format_is_yuvj(ctx->dst_fmt)) {
        return 0;
    }

    src_coefficients = sws_getCoefficients(video_frame_convert_sws_colorspace(src_frame));
    if (video_frame_convert_dst_is_limited_bt709(ctx)) {
        dst_coefficients = sws_getCoefficients(SWS_CS_ITU709);
        dst_range = 0;
    } else {
        dst_coefficients = src_coefficients;
        dst_range = 1;
    }
    src_range = video_frame_convert_frame_is_rgb(src_frame) ||
                src_frame->color_range == AVCOL_RANGE_JPEG ||
                video_frame_convert_format_is_yuvj(
                    (enum AVPixelFormat)src_frame->format);
...
static app_status_t video_frame_convert_alloc_work_frame(video_frame_convert_ctx_t *ctx) {
    AVBufferRef *buffer;
    size_t luma_size;
    size_t chroma_size;
    int chroma_height;
    ...
    if (ctx->dst_fmt == AV_PIX_FMT_NV12 ||
        ...
        if (first_alloc) {
            LOGD("encode contiguous %s buffer: size=%zu linesize=%d uv_offset=%zu",
...
#ifdef HAVE_LIBRGA
static app_status_t video_frame_convert_alloc_rga_resize_frame(video_frame_convert_ctx_t *ctx) {
...
static int video_frame_convert_rga_status_ok(IM_STATUS status) {
    return status == IM_STATUS_SUCCESS || status == IM_STATUS_NOERROR;
}

static int video_frame_convert_rga_pix_fmt(enum AVPixelFormat fmt) {
    switch (fmt) {
 ...
 */
static int video_frame_convert_rga_frame_usable(const AVFrame *frame) {
...
static int video_frame_convert_rga_plane_bpp(enum AVPixelFormat fmt) {
    switch (fmt) {
        case AV_PIX_FMT_YUYV422:
        case AV_PIX_FMT_UYVY422:
            return 2;
 ...
 * 与原先 swscale 默认行为一致。
 */
static const AVFrame *video_frame_convert_rga_convert(video_frame_convert_ctx_t *ctx,
                                                const AVFrame *src_frame,
                                                int *converted) {
    ...
    src_rga_fmt = video_frame_convert_rga_pix_fmt(src_av_fmt);
    dst_rga_fmt = video_frame_convert_rga_pix_fmt(ctx->dst_fmt);
    if (src_rga_fmt == RK_FORMAT_UNKNOWN || dst_rga_fmt == RK_FORMAT_UNKNOWN) {
        return NULL;
    }
    if (!video_frame_convert_rga_frame_usable(src_frame)) {
        return NULL;
    }
    if (video_frame_convert_alloc_work_frame(ctx) != APP_OK) {
        return NULL;
    }
    ...
    src_img = wrapbuffer_virtualaddr(src_frame->data[0],
                                     ...
                                     src_frame->height,
                                     src_rga_fmt,
                                     src_frame->linesize[0] / video_frame_convert_rga_plane_bpp(src_av_fmt),
                                     src_frame->height);
    dst_img = wrapbuffer_virtualaddr(ctx->work_frame->data[0],
                                     ctx->dst_width,
                                     ctx->dst_height,
                                     dst_rga_fmt,
                                     ctx->work_frame->linesize[0] / video_frame_convert_rga_plane_bpp(ctx->dst_fmt),
                                     ctx->dst_height);
    ...
    if (rgb_to_nv12 &&
        ...
        if (video_frame_convert_alloc_rga_resize_frame(ctx) != APP_OK) {
            return NULL;
        }
        ...
        resize_img = wrapbuffer_handle(ctx->rga_resize_handle,
                                       ...
                                       ctx->dst_height);
        status = video_frame_convert_rga_resize_on_core(src_img,
                                                  resize_img,
                                                  IM_SCHEDULER_RGA3_CORE0);
        if (video_frame_convert_rga_status_ok(status)) {
            ...
            status = video_frame_convert_rga_cvtcolor_on_core(
                resize_img,
                dst_img,
                RK_FORMAT_BGR_888,
                dst_rga_fmt,
    ...
    if (!ctx->rga_path_logged) {
        LOGD("encode convert path=%s src=%dx%d fmt=%d dst=%dx%d fmt=%d",
             rgb_to_nv12 &&
                     (src_frame->width != ctx->dst_width || src_frame->height != ctx->dst_height)
                 ? "RGA-two-pass-BT709"
                 : "RGA",
...
app_status_t video_frame_convert_init_hw(video_frame_convert_ctx_t *ctx,
    ...
    status = video_frame_convert_init(ctx, dst_width, dst_height, dst_sw_fmt);
    if (status != APP_OK) {
        return status;
    }
    ctx->output_is_hw = 1;
...
const AVFrame *video_frame_convert_prepare_rga_bgr_resize(video_frame_convert_ctx_t *ctx,
    ...
    if (ctx == NULL || src_frame == NULL ||
        ...
        !video_frame_convert_rga_frame_usable(src_frame) ||
        ctx->rga_failed) {
        return NULL;
    }
    if (video_frame_convert_alloc_rga_resize_frame(ctx) != APP_OK) {
    ...
    if (!ctx->rga_path_logged) {
        LOGD("compare resize path=RGA-BGR src=%dx%d dst=%dx%d",
             src_frame->width,
             src_frame->height,
             ctx->dst_width,
             ctx->dst_height);
...
const AVFrame *video_frame_convert_prepare(video_frame_convert_ctx_t *ctx,
...
#ifdef HAVE_LIBRGA
    {
        const AVFrame *filter_frame =
            video_rkrga_filter_convert(ctx, src_frame, converted);
    ...
    if (video_frame_convert_frame_is_hw(src_frame)) {
        convert_src = video_frame_convert_transfer_hw_frame(ctx, src_frame);
        if (convert_src == NULL) {
            return NULL;
        }
...
#ifdef HAVE_LIBRGA
    {
        ...
        const AVFrame *rga_frame = video_frame_convert_rga_convert(ctx, convert_src, converted);
        if (rga_frame != NULL) {
            return rga_frame;
        }
    ...
    if (ctx->sws == NULL ||
        ...
        ctx->sws = sws_getCachedContext(ctx->sws,
                                        convert_src->width,
                                        convert_src->height,
                                        video_frame_convert_sws_format(src_fmt),
                                        ctx->dst_width,
                                        ctx->dst_height,
                                        video_frame_convert_sws_format(ctx->dst_fmt),
                                        SWS_BILINEAR,
                                        NULL,
                                        NULL,
                                        NULL);
    ...
    status = video_frame_convert_alloc_work_frame(ctx);
    if (status != APP_OK) {
        return NULL;
    }

    ret = video_frame_convert_configure_sws(ctx, convert_src);
    if (ret < 0) {
    ...
    if (!ctx->sws_path_logged) {
        LOGD("encode convert path=swscale src=%dx%d fmt=%s(%d) "
             "dst=%dx%d fmt=%s(%d)",
             convert_src->width,
             convert_src->height,
             video_frame_convert_pix_fmt_name(src_fmt),
             src_fmt,
             ctx->dst_width,
             ctx->dst_height,
             video_frame_convert_pix_fmt_name(ctx->dst_fmt),
    ...
    if (video_frame_convert_dst_is_limited_bt709(ctx)) {
        ...
        ctx->work_frame->color_primaries = AVCOL_PRI_BT709;
        ctx->work_frame->color_trc = AVCOL_TRC_BT709;
    } else if (video_frame_convert_format_is_yuvj(ctx->dst_fmt)) {
        ctx->work_frame->color_range = AVCOL_RANGE_JPEG;
        ctx->work_frame->colorspace = convert_src->colorspace;
        ctx->work_frame->color_primaries = convert_src->color_primaries;
        ctx->work_frame->color_trc = convert_src->color_trc;
    }
    ctx->work_frame->pts = src_frame->pts;
    if (converted != NULL) {
        *converted = 1;
