...
app_status_t vp_control_osd_set_enabled(vp_control_t *control, int enabled) {
    ...
    pthread_mutex_lock(&control->lock);
    status = vp_osd_get(control->pipeline, &control->osd);
    if (status == APP_OK) {
...
app_status_t vp_control_osd_set_gimbal_angles(vp_control_t *control,
    ...
    pthread_mutex_lock(&control->lock);
    status = vp_osd_set_gimbal_angles(control->pipeline,
                                      yaw_deg,
                                      pitch_deg,
                                      valid);
    if (status == APP_OK) {
...
app_status_t vp_control_osd_set_laser_distance(vp_control_t *control,
    ...
    pthread_mutex_lock(&control->lock);
    status = vp_osd_set_laser_distance(control->pipeline,
                                       distance_m,
                                       signal_level,
                                       valid);
    if (status == APP_OK) {
...
app_status_t vp_control_osd_set_laser_continuous(vp_control_t *control, int active) {
    app_status_t status;
    ...
    pthread_mutex_lock(&control->lock);
    status = vp_osd_set_laser_continuous(control->pipeline, active);
    if (status == APP_OK) {
        status = vp_osd_get(control->pipeline, &control->osd);
    }
    pthread_mutex_unlock(&control->lock);
...
app_status_t vp_control_osd_set_laser_measuring(vp_control_t *control, int measuring) {
    app_status_t status;
    ...
    pthread_mutex_lock(&control->lock);
    status = vp_osd_set_laser_measuring(control->pipeline, measuring);
    if (status == APP_OK) {
        status = vp_osd_get(control->pipeline, &control->osd);
    }
    pthread_mutex_unlock(&control->lock);
...
app_status_t vp_control_osd_set_laser_error(vp_control_t *control, int error) {
    app_status_t status;
    ...
    pthread_mutex_lock(&control->lock);
    status = vp_osd_set_laser_error(control->pipeline, error);
    if (status == APP_OK) {
        status = vp_osd_get(control->pipeline, &control->osd);
    }
    pthread_mutex_unlock(&control->lock);
