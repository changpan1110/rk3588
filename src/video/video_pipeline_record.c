...
static app_status_t vp_record_open_output(vp_channel_t *ch,
    ...
    use_hw_frames = ch->input.source_type == VIDEO_SOURCE_HDMI_IN &&
                    warmup_frame != NULL;
    ...
    if ((use_hw_frames
             ? output_encode_convert_init_hw(&ch->rec_convert,
                                             width,
                                             height,
                                             AV_PIX_FMT_NV12)
             : output_encode_convert_init(&ch->rec_convert,
                                          width,
                                          height,
    ...
    if (use_hw_frames) {
        hw_encode_frame = output_encode_prepare_frame(&ch->rec_convert,
                                                      warmup_frame,
                                                      &converted);
    ...
    if (vp_open_h264_encoder(&ch->rec_enc,
                             ...
                             ch->rec_bitrate,
                             fps,
                             use_hw_frames ? AV_PIX_FMT_DRM_PRIME : AV_PIX_FMT_NV12,
    ...
    LOGI("%s record output opened %dx%d@%d bitrate=%d input=%s path=%s",
         ch->name,
         width,
         height,
         fps,
         ch->rec_bitrate,
         use_hw_frames ? "drm_prime/nv12" : "nv12",
         ch->rec_path);
    return APP_OK;
}
...
static app_status_t vp_record_encode_frame(vp_channel_t *ch,
    ...
    enc_frame = output_encode_prepare_frame(&ch->rec_convert, src_frame, &converted);
    if (out_convert_us != NULL) {
...
static void vp_record_close_output(vp_channel_t *ch) {
    ...
    output_encode_convert_deinit(&ch->rec_convert);
}
...
static void *vp_record_thread(void *opaque) {
    ...
    while (1) {
...

        {
            ...
            int64_t encode_started_us = app_get_time_us();
            ...
            status = vp_record_encode_frame(ch,
                                            ...
                                            &convert_us,
                                            &codec_mux_us);
            encode_sum_us += app_get_time_us() - encode_started_us;
        ...
        if (app_get_time_us() - timing_started_us >= 1000000) {
            ...
            pthread_mutex_unlock(&p->lock);
            LOGI("%s record timing frames=%d total=%.3fms convert=%.3fms "
                 "codec_mux=%.3fms queue=%d peak=%d dropped=%d",
                 ch->name,
                 timing_frames,
                 timing_frames > 0
                      ? (double)encode_sum_us / (double)timing_frames / 1000.0
                      : 0.0,
                 timing_frames > 0
                     ? (double)convert_sum_us / (double)timing_frames / 1000.0
                     : 0.0,
                 timing_frames > 0
                     ? (double)codec_mux_sum_us / (double)timing_frames / 1000.0
                     : 0.0,
                 queue_count,
                 queue_peak,
                 dropped);
...
app_status_t vp_record_start(vp_ctx_t *p, vp_channel_id_t id, const char *mp4_path) {
    ...
    LOGI("%s record preparing %dx%d@%d", ch->name, width, height, fps);
    ...
    if (warmup_frame != NULL) {
        ...
        prepared_frame = output_encode_prepare_frame(&ch->rec_convert,
                                                     warmup_frame,
                                                     &converted);
        ...
        warmup_encoded = 1;
        LOGI("%s record conversion+encoder warmup complete in %.3f ms",
             ch->name,
...
app_status_t vp_record_stop_all_ex(vp_ctx_t *p,
    ...
    for (i = 0; i < p->channel_count; ++i) {
        ...
        pthread_mutex_lock(&p->lock);
        active = p->ch[i].record_initializing ||
                 p->ch[i].record_wanted ||
                 p->ch[i].recording;
        pthread_mutex_unlock(&p->lock);
        if (active) {
            vp_record_stats_t channel_stats;

            if (vp_record_stop_ex(p, (vp_channel_id_t)i, &channel_stats) == APP_OK &&
                stats_count < stats_capacity) {
