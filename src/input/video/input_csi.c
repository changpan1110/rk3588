...
#define LOG_FILE_NAME "input_csi.c"
...
#define INPUT_CSI_NATIVE_BUFFER_COUNT 16
#define INPUT_CSI_DRM_FOURCC(a, b, c, d) \
    ((uint32_t)(a) | ((uint32_t)(b) << 8) | \
     ((uint32_t)(c) << 16) | ((uint32_t)(d) << 24))
#define INPUT_CSI_DRM_FORMAT_NV12 INPUT_CSI_DRM_FOURCC('N', 'V', '1', '2')
#define INPUT_CSI_DRM_FORMAT_NV16 INPUT_CSI_DRM_FOURCC('N', 'V', '1', '6')
#define INPUT_CSI_DRM_FORMAT_MOD_LINEAR 0ULL

/* ===================================================================== */
/* 原生 V4L2 DMA-BUF / DRM PRIME 零拷贝路径                              */
/* 与 input_hdmi_in.c 的原生路径同构,仅保留 NV12/NV16 格式。             */
/* ===================================================================== */

static int input_csi_native_format_requested(const char *format) {
    return format == NULL || format[0] == '\0' ||
           strcmp(format, "nv12") == 0 ||
           strcmp(format, "nv16") == 0;
}

static uint32_t input_csi_native_requested_fourcc(const char *format) {
...
static int input_csi_native_enabled(const video_input_config_t *cfg) {
    const char *value = getenv("RK_CSI_NATIVE_V4L2");
    ...
    LOGW("invalid RK_CSI_NATIVE_V4L2=%s; using code configuration=%d",
         value,
...
typedef struct {
    struct input_csi_native_ctx *owner;
    size_t length[VIDEO_MAX_PLANES];
    int dmabuf_fd[VIDEO_MAX_PLANES];
    AVRKMPPDRMFrameDescriptor drm_desc;
    unsigned int plane_count;
    unsigned int index;
} input_csi_native_buffer_t;

struct input_csi_native_ctx {
    int fd;
    ...
    input_csi_native_buffer_t buffers[INPUT_CSI_NATIVE_BUFFER_COUNT];
...
static int input_csi_ioctl(int fd, unsigned long request, void *arg) {
    int ret;
...
static void input_csi_native_destroy(input_csi_native_ctx_t *native) {
    unsigned int i;
...
static app_status_t input_csi_native_open_hw_frames(input_csi_native_ctx_t *native) {
    ...
    ret = av_hwdevice_ctx_create(&native->hw_device_ctx,
                                 ...
                                 0);
    if (ret < 0) {
        LOGE("csi create RKMPP hardware device failed: %d", ret);
    ...
    if (ret < 0) {
        LOGE("csi initialize DRM PRIME/%s frames context failed: %d",
...
static void input_csi_native_init_drm_descriptor(input_csi_native_ctx_t *native,
                                                 input_csi_native_buffer_t *buffer) {
    ...
    for (plane = 0; plane < buffer->plane_count; ++plane) {
        ...
        desc->objects[plane].format_modifier = INPUT_CSI_DRM_FORMAT_MOD_LINEAR;
    }
    desc->nb_layers = 1;
...
static int input_csi_native_configure_pixel_format(input_csi_native_ctx_t *native) {
    switch (native->pixel_format) {
        ...
        case V4L2_PIX_FMT_NV12M:
            ...
            native->drm_format = INPUT_CSI_DRM_FORMAT_NV12;
        ...
        case V4L2_PIX_FMT_NV16M:
            ...
            native->drm_format = INPUT_CSI_DRM_FORMAT_NV16;
            native->drm_plane_count = 2;
            break;
        default:
            return 0;
...
static void input_csi_native_configure_color(input_csi_native_ctx_t *native,
                                             uint32_t colorspace,
...
static int input_csi_native_validate_buffer_layout(
    const input_csi_native_ctx_t *native,
    const input_csi_native_buffer_t *buffer) {
    uint64_t luma_size;
...
static int input_csi_native_queue_locked(input_csi_native_ctx_t *native,
                                         unsigned int index) {
    ...
    return input_csi_ioctl(native->fd, VIDIOC_QBUF, &buffer);
}

static void input_csi_native_buffer_release(void *opaque, uint8_t *data) {
    input_csi_native_buffer_t *buffer = (input_csi_native_buffer_t *)opaque;
    ...
    if (!native->closing && native->fd >= 0) {
        if (input_csi_native_queue_locked(native, buffer->index) < 0) {
            LOGE("csi native QBUF index=%u failed: %s",
                 buffer->index,
                 strerror(errno));
        }
    }
    ...
    if (destroy) {
        input_csi_native_destroy(native);
    }
}

static app_status_t input_csi_native_open(input_csi_ctx_t *ctx) {
    input_csi_native_ctx_t *native;
    ...
    native = (input_csi_native_ctx_t *)calloc(1, sizeof(*native));
    if (native == NULL) {
        return APP_ERR_NOMEM;
    }
    native->fd = -1;
    native->refs = 1;
    for (i = 0; i < INPUT_CSI_NATIVE_BUFFER_COUNT; ++i) {
        unsigned int plane;
    ...
    if (native->fd < 0) {
        LOGE("csi native open %s failed: %s", ctx->cfg.device, strerror(errno));
        input_csi_native_destroy(native);
        return APP_ERR_IO;
    }

    memset(&capability, 0, sizeof(capability));
    if (input_csi_ioctl(native->fd, VIDIOC_QUERYCAP, &capability) < 0) {
        LOGE("csi native VIDIOC_QUERYCAP failed: %s", strerror(errno));
        goto fail_io;
    }
    ...
    if (device_caps & V4L2_CAP_VIDEO_CAPTURE_MPLANE) {
        ...
        LOGW("csi native device has no capture capability");
        goto fail_unsupported;
    }
    if (!(device_caps & V4L2_CAP_STREAMING)) {
        LOGW("csi native device has no streaming capability");
        goto fail_unsupported;
    }

    memset(&format, 0, sizeof(format));
    format.type = native->buffer_type;
    if (input_csi_ioctl(native->fd, VIDIOC_G_FMT, &format) < 0) {
        LOGE("csi native VIDIOC_G_FMT failed: %s", strerror(errno));
        goto fail_io;
    }

    requested_fourcc = input_csi_native_requested_fourcc(ctx->cfg.input_format);
    ...
    if (requested_fourcc != 0 || requested_width != 0 || requested_height != 0) {
        ...
        if (input_csi_ioctl(native->fd, VIDIOC_S_FMT, &format) < 0) {
            LOGW("csi native requested format %s %ux%u failed: %s; keeping driver format",
            ...
            format.type = native->buffer_type;
            if (input_csi_ioctl(native->fd, VIDIOC_G_FMT, &format) < 0) {
                LOGE("csi native VIDIOC_G_FMT after S_FMT failure failed: %s",
                     strerror(errno));
                goto fail_io;
            }
        }
    ...
    if (native->buffer_type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
        ...
        if (native->plane_count == 0 || native->plane_count > VIDEO_MAX_PLANES) {
            LOGE("csi native invalid plane count: %u", native->plane_count);
            goto fail_unsupported;
        }
    ...
    if (!input_csi_native_configure_pixel_format(native)) {
        LOGW("csi native unsupported format fourcc=%c%c%c%c memory_planes=%u",
    ...
    input_csi_native_configure_color(native, colorspace, quantization);
    if (input_csi_native_open_hw_frames(native) != APP_OK) {
        goto fail_unsupported;
    }

    memset(&request, 0, sizeof(request));
    request.count = INPUT_CSI_NATIVE_BUFFER_COUNT;
    request.type = native->buffer_type;
    request.memory = V4L2_MEMORY_MMAP;
    if (input_csi_ioctl(native->fd, VIDIOC_REQBUFS, &request) < 0 || request.count == 0) {
        LOGE("csi native VIDIOC_REQBUFS failed: %s", strerror(errno));
        goto fail_io;
    }
    native->buffer_count = request.count > INPUT_CSI_NATIVE_BUFFER_COUNT
                               ? INPUT_CSI_NATIVE_BUFFER_COUNT
                               : request.count;
    ...
    for (i = 0; i < native->buffer_count; ++i) {
        ...
        if (input_csi_ioctl(native->fd, VIDIOC_QUERYBUF, &buffer) < 0) {
            LOGE("csi native QUERYBUF index=%u failed: %s", i, strerror(errno));
            goto fail_io;
        }
        ...
        if (!input_csi_native_validate_buffer_layout(native, &native->buffers[i])) {
            LOGE("csi native buffer layout too small: index=%u size=%zu/%zu",
                 i,
        ...
        for (plane = 0; plane < native->plane_count; ++plane) {
            ...
            if (input_csi_ioctl(native->fd, VIDIOC_EXPBUF, &export_buffer) < 0) {
                LOGW("csi native EXPBUF index=%u plane=%u failed: %s",
                     i,
                     plane,
                     strerror(errno));
                goto fail_unsupported;
        ...
        input_csi_native_init_drm_descriptor(native, &native->buffers[i]);
        if (input_csi_native_queue_locked(native, i) < 0) {
            LOGE("csi native QBUF index=%u failed: %s", i, strerror(errno));
            goto fail_io;
        }
    }

    if (input_csi_ioctl(native->fd, VIDIOC_STREAMON, &native->buffer_type) < 0) {
        LOGE("csi native STREAMON failed: %s", strerror(errno));
        goto fail_io;
    }
    native->streaming = 1;
    ctx->native = native;
    ...
    LOGI("csi native V4L2 DMA-BUF opened: %ux%u fourcc=%c%c%c%c sw_fmt=%s "
...
fail_io:
    if (native->streaming) {
        input_csi_ioctl(native->fd, VIDIOC_STREAMOFF, &native->buffer_type);
    }
    if (native->fd >= 0) {
        close(native->fd);
        native->fd = -1;
...
static app_status_t input_csi_native_read(input_csi_ctx_t *ctx, AVFrame *frame) {
    input_csi_native_ctx_t *native = ctx->native;
    struct pollfd poll_fd;
    struct v4l2_buffer buffer;
    struct v4l2_plane planes[VIDEO_MAX_PLANES];
    input_csi_native_buffer_t *native_buffer;
    ...
    if (poll_ret < 0 || (poll_fd.revents & (POLLERR | POLLHUP | POLLNVAL))) {
        LOGE("csi native poll failed: %s revents=0x%x",
    ...
    if (input_csi_ioctl(native->fd, VIDIOC_DQBUF, &buffer) < 0) {
        saved_errno = errno;
        pthread_mutex_unlock(&native->lock);
        if (saved_errno == EAGAIN) {
            return APP_ERR_EOF;
        }
        LOGE("csi native DQBUF failed: %s", strerror(saved_errno));
        return APP_ERR_IO;
    }
    if (buffer.index >= native->buffer_count) {
        pthread_mutex_unlock(&native->lock);
        LOGE("csi native invalid buffer index: %u", buffer.index);
        return APP_ERR_IO;
    }
    native->refs++;
    pthread_mutex_unlock(&native->lock);
    ...
    buffer_ref = av_buffer_create((uint8_t *)&native_buffer->drm_desc,
                                  ...
                                  input_csi_native_buffer_release,
                                  native_buffer,
                                  AV_BUFFER_FLAG_READONLY);
    if (buffer_ref == NULL) {
        input_csi_native_buffer_release(native_buffer,
    ...
    if (native->frame_count == 1) {
        LOGD("csi native DMA-BUF frame count=%llu buffer=%u fd=%d "
...
static void input_csi_native_close(input_csi_ctx_t *ctx) {
    input_csi_native_ctx_t *native = ctx->native;
    int destroy = 0;
    ...
    if (destroy) {
        ...
        LOGW("csi native close deferred: frames are still referenced");
    }
    ctx->native = NULL;
    ctx->use_native = 0;
}
...
/* ===================================================================== */
/* FFmpeg v4l2 软件帧路径(兼容旧 input_csi0/1)                           */
/* ===================================================================== */

static int input_csi_is_rawvideo(enum AVCodecID codec_id) {
    return codec_id == AV_CODEC_ID_RAWVIDEO;
}

static int input_csi_is_raw_v4l2_format(const char *fmt) {
    if (fmt == NULL || fmt[0] == '\0') {
        return 0;
    }
...
static const AVCodec *input_csi_select_decoder(enum AVCodecID codec_id) {
    const AVCodec *decoder = NULL;

    if (input_csi_is_rawvideo(codec_id)) {
        decoder = avcodec_find_decoder(codec_id);
        if (decoder != NULL) {
            LOGD("csi rawvideo path: %s", decoder->name);
        }
        return decoder;
    }
    ...
    switch (codec_id) {
        case AV_CODEC_ID_H264:
            ...
            if (decoder != NULL) {
                LOGD("csi decoder select: h264_rkmpp");
                return decoder;
            }
            break;
        case AV_CODEC_ID_HEVC:
            ...
            if (decoder != NULL) {
                LOGD("csi decoder select: hevc_rkmpp");
                return decoder;
            }
            break;
        case AV_CODEC_ID_MJPEG:
            ...
            if (decoder != NULL) {
                LOGD("csi decoder select: mjpeg_rkmpp");
                return decoder;
            }
            break;
        default:
    ...
    decoder = avcodec_find_decoder(codec_id);
    if (decoder != NULL) {
        LOGD("csi decoder fallback: %s", decoder->name);
    }
    return decoder;
}

static app_status_t input_csi_open_decoder(input_csi_ctx_t *ctx) {
    AVStream *stream;
    const AVCodec *decoder;
    int ret;

    stream = ctx->decoder.fmt_ctx->streams[ctx->decoder.video_stream_index];
    if (input_csi_is_rawvideo(stream->codecpar->codec_id)) {
        LOGD("csi stream codec is rawvideo, skip rkmpp compressed decoder selection");
    }
    decoder = input_csi_select_decoder(stream->codecpar->codec_id);
    if (decoder == NULL) {
        LOGE("csi decoder not found for codec id=%d", stream->codecpar->codec_id);
        return APP_ERR_UNSUPPORTED;
    }
    ...
    if (ret < 0) {
        LOGE("csi avcodec_parameters_to_context failed: %d", ret);
        return APP_ERR_FFMPEG;
    }

    ret = avcodec_open2(ctx->decoder.dec_ctx, decoder, NULL);
    if (ret < 0) {
        LOGE("csi avcodec_open2 failed: %d", ret);
        return APP_ERR_FFMPEG;
    }
...
static app_status_t input_csi_open_device(input_csi_ctx_t *ctx) {
    char video_size[32];
    char fps[16];
    ...
    if (input_fmt == NULL) {
        LOGE("csi video4linux2 input format not found");
        return APP_ERR_UNSUPPORTED;
    }
    ...
    if (ctx->cfg.fps > 0 && !input_csi_is_raw_v4l2_format(ctx->cfg.input_format)) {
    ...
    if (ret < 0) {
        LOGE("csi avformat_open_input failed for %s: %d", ctx->cfg.device, ret);
        return APP_ERR_FFMPEG;
    }

    ret = av_find_best_stream(ctx->decoder.fmt_ctx, AVMEDIA_TYPE_VIDEO, -1, -1, NULL, 0);
    if (ret < 0) {
        LOGE("csi av_find_best_stream failed: %d", ret);
        return APP_ERR_FFMPEG;
    }
    ...
    return input_csi_open_decoder(ctx);
}

static app_status_t input_csi_read_decoder(input_csi_ctx_t *ctx, AVFrame *frame) {
    int ret;
    ...
    while ((ret = av_read_frame(ctx->decoder.fmt_ctx, ctx->decoder.packet)) >= 0) {
        ...
        av_packet_unref(ctx->decoder.packet);
        if (ret < 0) {
            LOGE("csi avcodec_send_packet failed: %d", ret);
            return APP_ERR_FFMPEG;
        }
        ...
        LOGE("csi avcodec_receive_frame failed: %d", ret);
        return APP_ERR_FFMPEG;
    }
...
static app_status_t input_csi_open_ffmpeg(input_csi_ctx_t *ctx) {
    app_status_t status;

    status = input_csi_open_device(ctx);
    if (status != APP_OK) {
        input_csi_close(ctx);
        return status;
    }
...
app_status_t input_csi_open(input_csi_ctx_t *ctx, const video_input_config_t *cfg) {
    ...
    *ctx = (input_csi_ctx_t){0};
    ctx->cfg = *cfg;
    LOGD("open csi input (ffmpeg): %s %dx%d@%d fmt=%s",
         ctx->cfg.device, ctx->cfg.width, ctx->cfg.height,
         ctx->cfg.fps, ctx->cfg.input_format);
    return input_csi_open_ffmpeg(ctx);
}
...
app_status_t input_csi_open_hw(input_csi_ctx_t *ctx, const video_input_config_t *cfg) {
    ...
    *ctx = (input_csi_ctx_t){0};
    ctx->cfg = *cfg;
    LOGD("open csi input: %s %dx%d@%d fmt=%s native=%s",
         ctx->cfg.device, ctx->cfg.width, ctx->cfg.height, ctx->cfg.fps,
         ctx->cfg.input_format, ctx->cfg.use_native_v4l2 ? "yes" : "no");

    if (!input_csi_native_enabled(&ctx->cfg)) {
        LOGD("csi native V4L2 path disabled by configuration/environment");
    } else if (input_csi_native_format_requested(ctx->cfg.input_format)) {
        status = input_csi_native_open(ctx);
        if (status == APP_OK) {
            ctx->is_opened = 1;
            return APP_OK;
        }
        LOGW("csi native V4L2 path unavailable (%s), falling back to FFmpeg",
             app_status_str(status));
    } else {
        LOGD("csi requested format %s uses FFmpeg input path",
...
app_status_t input_csi_read(input_csi_ctx_t *ctx, video_frame_t *frame) {
    ...
    if (ctx->use_native) {
        status = input_csi_native_read(ctx, frame->av_frame);
    } else {
        status = input_csi_read_decoder(ctx, frame->av_frame);
    }
    if (status != APP_OK) {
        return status;
    }
    ...
    frame->source_type = ctx->cfg.source_type;
    strncpy(frame->source_name,
            video_source_type_str(ctx->cfg.source_type),
            sizeof(frame->source_name) - 1);
