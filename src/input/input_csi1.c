...
#define LOG_FILE_NAME "input_csi1.c"
...
static int input_csi1_is_raw_v4l2_format(const char *fmt) {
    if (fmt == NULL || fmt[0] == '\0') {
        return 0;
    }
...
static int input_csi1_is_rawvideo(enum AVCodecID codec_id) {
    return codec_id == AV_CODEC_ID_RAWVIDEO;
}

static const AVCodec *input_csi1_select_decoder(enum AVCodecID codec_id) {
    const AVCodec *decoder = NULL;

    if (input_csi1_is_rawvideo(codec_id)) {
        decoder = avcodec_find_decoder(codec_id);
        if (decoder != NULL) {
            LOGI("csi1 rawvideo path: %s", decoder->name);
        }
        return decoder;
    }
    ...
    switch (codec_id) {
        case AV_CODEC_ID_H264:
            ...
            if (decoder != NULL) {
                LOGI("csi1 decoder select: h264_rkmpp");
                return decoder;
            }
            break;
        case AV_CODEC_ID_HEVC:
            ...
            if (decoder != NULL) {
                LOGI("csi1 decoder select: hevc_rkmpp");
                return decoder;
            }
            break;
        case AV_CODEC_ID_MJPEG:
            ...
            if (decoder != NULL) {
                LOGI("csi1 decoder select: mjpeg_rkmpp");
                return decoder;
            }
            break;
        default:
    ...
    decoder = avcodec_find_decoder(codec_id);
    if (decoder != NULL) {
        LOGI("csi1 decoder fallback: %s", decoder->name);
    }
    return decoder;
}

static app_status_t input_csi1_open_decoder(input_csi1_ctx_t *ctx) {
    AVStream *stream;
    const AVCodec *decoder;
    int ret;

    stream = ctx->decoder.fmt_ctx->streams[ctx->decoder.video_stream_index];
    if (input_csi1_is_rawvideo(stream->codecpar->codec_id)) {
        LOGI("csi1 stream codec is rawvideo, skip rkmpp compressed decoder selection");
    }
    decoder = input_csi1_select_decoder(stream->codecpar->codec_id);
    if (decoder == NULL) {
        LOGE("csi1 decoder not found for codec id=%d", stream->codecpar->codec_id);
    ...
    if (ret < 0) {
        LOGE("csi1 avcodec_parameters_to_context failed: %d", ret);
        return APP_ERR_FFMPEG;
    }

    ret = avcodec_open2(ctx->decoder.dec_ctx, decoder, NULL);
    if (ret < 0) {
        LOGE("csi1 avcodec_open2 failed: %d", ret);
        return APP_ERR_FFMPEG;
    }
...
static app_status_t input_csi1_open_device(input_csi1_ctx_t *ctx) {
    char video_size[32];
    char fps[16];
    ...
    if (input_fmt == NULL) {
        LOGE("csi1 video4linux2 input format not found");
        return APP_ERR_UNSUPPORTED;
    }
    ...
    if (input_csi1_is_raw_v4l2_format(ctx->cfg.input_format)) {
        LOGI("csi1 raw v4l2 path: skip framerate/input_format override for %s", ctx->cfg.input_format);
    } else {
        av_dict_set(&options, "framerate", fps, 0);
    }
    if (ctx->cfg.input_format[0] != '\0' && !input_csi1_is_raw_v4l2_format(ctx->cfg.input_format)) {
    ...
    if (ret < 0) {
        LOGE("csi1 avformat_open_input failed for %s: %d", ctx->cfg.device, ret);
        return APP_ERR_FFMPEG;
    }
    ...
    if (ret < 0) {
        LOGE("csi1 av_find_best_stream failed: %d", ret);
        return APP_ERR_FFMPEG;
    }
    ...
    return input_csi1_open_decoder(ctx);
}

static app_status_t input_csi1_read_decoder(input_csi1_ctx_t *ctx, AVFrame *frame) {
    int ret;
    ...
    while ((ret = av_read_frame(ctx->decoder.fmt_ctx, ctx->decoder.packet)) >= 0) {
        ...
        av_packet_unref(ctx->decoder.packet);
        if (ret < 0) {
            LOGE("csi1 avcodec_send_packet failed: %d", ret);
            return APP_ERR_FFMPEG;
        }
        ...
        LOGE("csi1 avcodec_receive_frame failed: %d", ret);
        return APP_ERR_FFMPEG;
    }
...
app_status_t input_csi1_open(input_csi1_ctx_t *ctx, const video_input_config_t *cfg) {
    ...
    *ctx = (input_csi1_ctx_t){0};
    ctx->cfg = *cfg;
    LOGI("open csi1 input: %s %dx%d@%d fmt=%s",
        ctx->cfg.device, ctx->cfg.width, ctx->cfg.height, ctx->cfg.fps, ctx->cfg.input_format);
    status = input_csi1_open_device(ctx);
    if (status != APP_OK) {
        ctx->is_opened = 0;
        return status;
    }
...
app_status_t input_csi1_read(input_csi1_ctx_t *ctx, video_frame_t *frame) {
    ...
    status = input_csi1_read_decoder(ctx, frame->av_frame);
    if (status != APP_OK) {
        return status;
    }

    frame->pts_us = app_get_time_us();
    frame->source_type = VIDEO_SOURCE_CSI1;
    strncpy(frame->source_name, "csi1", sizeof(frame->source_name) - 1);
