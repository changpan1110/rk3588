...
/* USB MJPEG input requires the Rockchip MPP hardware decoder. */
#define INPUT_USB_EXTRA_HW_FRAMES 16
...
static void input_usb_log_hw_frame(input_usb_ctx_t *ctx, const AVFrame *frame) {
    ...
    ++ctx->hw_frame_count;
    if (ctx->hw_frame_count != 1 && ctx->hw_frame_count % 120 != 0) {
        return;
    }
...
static app_status_t input_usb_transfer_hw_frame(input_usb_ctx_t *ctx, AVFrame *frame) {
    ...
    if (ctx->hw_transfer_count == 1 || ctx->hw_transfer_count % 120 == 0) {
...
static const AVCodec *input_usb_select_decoder(enum AVCodecID codec_id) {
    ...
    if (codec_id == AV_CODEC_ID_MJPEG) {
        ...
        if (decoder == NULL) {
            LOGE("usb hardware decoder mjpeg_rkmpp not found");
            return NULL;
        }

        LOGI("usb decoder select hardware: %s", decoder->name);
        return decoder;
    }

    decoder = avcodec_find_decoder(codec_id);
    if (decoder != NULL) {
        LOGI("usb decoder select software: %s", decoder->name);
    }
    return decoder;
}
...
static app_status_t input_usb_open_decoder(input_usb_ctx_t *ctx) {
    ...
    if (ctx->retain_hw_frames && strcmp(decoder->name, "mjpeg_rkmpp") == 0) {
        ctx->decoder.dec_ctx->extra_hw_frames = INPUT_USB_EXTRA_HW_FRAMES;
    }
    ...
    LOGI("usb decoder opened: %s extra_hw_frames=%d",
         decoder->name,
...
static app_status_t input_usb_open_internal(input_usb_ctx_t *ctx,
    ...
    *ctx = (input_usb_ctx_t){0};
    ...
    LOGI("open usb input: %s %dx%d@%d fmt=%s hw_frames=%s",
         ctx->cfg.device,
         ctx->cfg.width,
         ctx->cfg.height,
         ctx->cfg.fps,
