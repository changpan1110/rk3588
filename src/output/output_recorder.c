...
#define LOG_FILE_NAME "output_recorder.c"
...
#define OUTPUT_RECORDER_QUEUE_SIZE 8
#define OUTPUT_RECORDER_DEFAULT_BITRATE 4000000
#define OUTPUT_RECORDER_DEFAULT_DIR "/tmp"

struct output_recorder_ctx {
    ...
    AVFrame *queue_frames[OUTPUT_RECORDER_QUEUE_SIZE];
    int queue_head;
    int queue_count;
...
static void output_recorder_emit(output_recorder_ctx_t *ctx, output_recorder_event_t event, const char *path) {
...
static int output_recorder_mkdir_p(const char *dir) {
...
static void output_recorder_make_auto_path(output_recorder_ctx_t *ctx,
                                           char *out,
                                           size_t out_size,
                                           const char *prefix,
                                           const char *ext) {
...
static app_status_t output_recorder_open_input(output_recorder_ctx_t *ctx) {
    switch (ctx->cfg.input.source_type) {
        ...
        case VIDEO_SOURCE_CSI0:
            return input_csi0_open(&ctx->input.csi0, &ctx->cfg.input);
        case VIDEO_SOURCE_CSI1:
            return input_csi1_open(&ctx->input.csi1, &ctx->cfg.input);
...
static app_status_t output_recorder_read_input(output_recorder_ctx_t *ctx, video_frame_t *frame) {
    switch (ctx->cfg.input.source_type) {
        case VIDEO_SOURCE_USB:
...
static void output_recorder_close_input(output_recorder_ctx_t *ctx) {
    switch (ctx->cfg.input.source_type) {
        case VIDEO_SOURCE_USB:
...
static app_status_t output_recorder_queue_push(output_recorder_ctx_t *ctx, const AVFrame *frame) {
    AVFrame *dst_frame;
    int slot;
    int ret;

    pthread_mutex_lock(&ctx->queue_lock);
    if (ctx->queue_count == OUTPUT_RECORDER_QUEUE_SIZE) {
        /* 队列满丢最老的一帧,保实时性 */
        slot = ctx->queue_head;
        if (ctx->queue_frames[slot] != NULL) {
            av_frame_unref(ctx->queue_frames[slot]);
        }
        ctx->queue_head = (ctx->queue_head + 1) % OUTPUT_RECORDER_QUEUE_SIZE;
        ctx->queue_count--;
        ctx->dropped_frames++;
    }

    slot = (ctx->queue_head + ctx->queue_count) % OUTPUT_RECORDER_QUEUE_SIZE;
    if (ctx->queue_frames[slot] == NULL) {
 ...
 */
static app_status_t output_recorder_queue_pop(output_recorder_ctx_t *ctx, AVFrame *dst_frame) {
    AVFrame *src_frame;
    int ret;
...
static const AVCodec *output_recorder_find_encoder(const char *preferred_name) {
    const AVCodec *codec;
...
static app_status_t output_recorder_open_output(output_recorder_ctx_t *ctx, int width, int height, int fps) {
    int ret;

    ctx->codec = output_recorder_find_encoder(ctx->encoder_name);
    if (ctx->codec == NULL) {
    ...
    LOGI("recorder output opened encoder=%s %dx%d@%d bitrate=%d path=%s",
         ctx->codec->name,
         width,
         height,
         fps > 0 ? fps : 30,
...
static app_status_t output_recorder_write_packets(output_recorder_ctx_t *ctx) {
    int ret;
...
static app_status_t output_recorder_encode_frame(output_recorder_ctx_t *ctx, AVFrame *src_frame) {
    ...
    return output_recorder_write_packets(ctx);
}

static app_status_t output_recorder_flush_encoder(output_recorder_ctx_t *ctx) {
    int ret;
 ...
 */
static void output_recorder_finalize_record(output_recorder_ctx_t *ctx) {
    int64_t duration_us;
    ...
    if (ctx->codec_ctx != NULL) {
        if (output_recorder_flush_encoder(ctx) != APP_OK) {
    ...
    if (ctx->encoded_frames > 0) {
        ...
        output_recorder_emit(ctx, OUTPUT_RECORDER_EVENT_RECORD_FINISHED, ctx->record_path);
    } else {
        LOGI("record canceled: no frames encoded, no file written");
        output_recorder_emit(ctx, OUTPUT_RECORDER_EVENT_RECORD_CANCELED, NULL);
    }
}

static void *output_recorder_capture_thread(void *opaque) {
    output_recorder_ctx_t *ctx = (output_recorder_ctx_t *)opaque;
    video_frame_t frame;
    app_status_t status;
    int want_push;
    ...
    while (!ctx->stop_requested) {
        ...
        status = output_recorder_read_input(ctx, &frame);
        if (status != APP_OK) {
        ...
        pthread_mutex_unlock(&ctx->queue_lock);
        if (want_push) {
            status = output_recorder_queue_push(ctx, frame.av_frame);
            if (status != APP_OK) {
...
static void *output_recorder_encode_thread(void *opaque) {
    output_recorder_ctx_t *ctx = (output_recorder_ctx_t *)opaque;
    ...
    while (1) {
        ...
        if (emit_started) {
            output_recorder_emit(ctx, OUTPUT_RECORDER_EVENT_RECORD_STARTED, started_path);
        }

        status = output_recorder_queue_pop(ctx, encode_frame);
        if (status == APP_OK) {
            pthread_mutex_lock(&ctx->queue_lock);
            ...
            if (is_recording) {
                if (ctx->codec_ctx == NULL) {
                    status = output_recorder_open_output(ctx,
                                                         encode_frame->width,
                                                         encode_frame->height,
                                                         fps);
                    if (status != APP_OK) {
                        LOGE("recorder open output failed: %s", app_status_str(status));
                        output_recorder_emit(ctx, OUTPUT_RECORDER_EVENT_ERROR, ctx->record_path);
                        pthread_mutex_lock(&ctx->queue_lock);
                        ctx->record_stop_request = 1;
                        pthread_mutex_unlock(&ctx->queue_lock);
                        continue;
                ...
                status = output_recorder_encode_frame(ctx, encode_frame);
                if (status != APP_OK) {
        ...
        if (finalize) {
            output_recorder_finalize_record(ctx);
        }
...
app_status_t output_recorder_init(output_recorder_ctx_t **out_ctx, const output_recorder_config_t *cfg) {
    output_recorder_ctx_t *ctx;
    app_status_t status;
    ...
    ctx = (output_recorder_ctx_t *)calloc(1, sizeof(*ctx));
    if (ctx == NULL) {
        return APP_ERR_NOMEM;
    }
    ...
    if (cfg->storage_dir != NULL && cfg->storage_dir[0] != '\0') {
        ...
        strncpy(ctx->storage_dir, OUTPUT_RECORDER_DEFAULT_DIR, sizeof(ctx->storage_dir) - 1);
    }
    if (output_recorder_mkdir_p(ctx->storage_dir) != 0) {
        LOGW("recorder storage dir %s create failed: %s (record/snapshot may fail)",
             ctx->storage_dir,
             strerror(errno));
    }
    if (ctx->cfg.bitrate <= 0) {
        ctx->cfg.bitrate = OUTPUT_RECORDER_DEFAULT_BITRATE;
    }
    if (ctx->cfg.gop <= 0) {
        ctx->cfg.gop = cfg->input.fps > 0 ? cfg->input.fps : 30;
    }

    status = output_recorder_open_input(ctx);
    if (status != APP_OK) {
    ...
    if (pthread_mutex_init(&ctx->queue_lock, NULL) != 0 ||
        ...
        output_recorder_close_input(ctx);
        free(ctx);
        return APP_ERR_IO;
    }

    if (pthread_create(&ctx->capture_thread, NULL, output_recorder_capture_thread, ctx) != 0) {
        LOGE("recorder pthread_create capture failed");
        output_recorder_close_input(ctx);
        free(ctx);
        return APP_ERR_IO;
    }
    if (pthread_create(&ctx->encode_thread, NULL, output_recorder_encode_thread, ctx) != 0) {
...
app_status_t output_recorder_record_start(output_recorder_ctx_t *ctx, const char *mp4_path) {
    if (ctx == NULL) {
        return APP_ERR_PARAM;
    }
    ...
    if (mp4_path != NULL && mp4_path[0] != '\0') {
        ...
        output_recorder_make_auto_path(ctx, ctx->pending_record_path, sizeof(ctx->pending_record_path), "record", ".mp4");
    }
...
app_status_t output_recorder_record_stop(output_recorder_ctx_t *ctx) {
    if (ctx == NULL) {
        return APP_ERR_PARAM;
    }
...
app_status_t output_recorder_write_jpeg(const AVFrame *src_frame, const char *path) {
    output_encode_convert_ctx_t hw_to_nv12;
    output_encode_convert_ctx_t jpeg_convert;
    ...
    if (frame->format == AV_PIX_FMT_DRM_PRIME) {
        ret = output_encode_convert_init(&hw_to_nv12,
                                         frame->width,
                                         frame->height,
                                         AV_PIX_FMT_NV12);
        if (ret != APP_OK) {
        ...
        jpeg_source = output_encode_prepare_frame(&hw_to_nv12, frame, &converted);
    ...
    converted = 0;
    ret = output_encode_convert_init(&jpeg_convert,
                                     jpeg_source->width,
                                     jpeg_source->height,
                                     AV_PIX_FMT_YUVJ420P);
    if (ret != APP_OK) {
    ...
    yuvj_frame = output_encode_prepare_frame(&jpeg_convert, jpeg_source, &converted);
    if (yuvj_frame == NULL) {
...
out:
    ...
    output_encode_convert_deinit(&jpeg_convert);
    output_encode_convert_deinit(&hw_to_nv12);
    av_frame_free(&frame);
    return result;
}

app_status_t output_recorder_snapshot(output_recorder_ctx_t *ctx, const char *jpg_path) {
    AVFrame *frame = NULL;
    app_status_t result;
    ...
    if (jpg_path != NULL && jpg_path[0] != '\0') {
        ...
        output_recorder_make_auto_path(ctx, path, sizeof(path), "snapshot", ".jpg");
    }
    ...
    result = output_recorder_write_jpeg(frame, path);
    av_frame_free(&frame);
    pthread_mutex_unlock(&ctx->snapshot_lock);

    if (result == APP_OK) {
        LOGI("snapshot saved: %s", path);
        output_recorder_emit(ctx, OUTPUT_RECORDER_EVENT_SNAPSHOT_SAVED, path);
    } else {
        output_recorder_emit(ctx, OUTPUT_RECORDER_EVENT_ERROR, path);
    }
    return result;
}

int output_recorder_is_recording(output_recorder_ctx_t *ctx) {
    int recording;
...
void output_recorder_deinit(output_recorder_ctx_t *ctx) {
    int i;
    ...
    output_encode_convert_deinit(&ctx->nv12_convert);
    for (i = 0; i < OUTPUT_RECORDER_QUEUE_SIZE; ++i) {
    ...
    LOGI("recorder deinit captured=%d dropped=%d", ctx->captured_frames, ctx->dropped_frames);
    free(ctx);
}
