...
#define LOG_FILE_NAME "rtp_output.c"
...
#include "output/stream/rtp/rtp_output.h"

#define OUTPUT_RTP_MTU 1400

static void rtp_output_write_rtp_header(uint8_t *buffer,
                                                int marker,
                                                uint16_t sequence,
                                                uint32_t timestamp,
                                                uint32_t ssrc) {
...
static app_status_t rtp_output_send_datagram(rtp_output_ctx_t *ctx,
...
static app_status_t rtp_output_send_nal(rtp_output_ctx_t *ctx,
                                                const uint8_t *nal,
                                                size_t nal_size,
                                                uint32_t timestamp,
                                                int marker) {
    ...
    if (nal_size <= payload_capacity) {
        rtp_output_write_rtp_header(buffer,
                                           marker,
                                           ctx->sequence,
                                           timestamp,
                                           ctx->ssrc);
        ...
        ctx->sequence++;
        return rtp_output_send_datagram(ctx, buffer, 12 + nal_size);
    }

    {
        ...
        while (remaining > 0) {
            ...
            ctx->sequence++;
            status = rtp_output_send_datagram(ctx, buffer, 14 + chunk);
            if (status != APP_OK) {
                return status;
            }
            position += chunk;
...
app_status_t rtp_output_send(rtp_output_ctx_t *ctx,
    ...
    while (current + 4 <= end) {
        ...
        if (nal_end > nal_start) {
            status = rtp_output_send_nal(ctx,
                                                nal_start,
