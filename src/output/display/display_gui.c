...
#define LOG_FILE_NAME "display_gui.c"
...
static int display_gui_get_direct_texture_format(enum AVPixelFormat fmt, Uint32 *texture_fmt) {
    if (texture_fmt == NULL) {
        return 0;
    }
...
static int display_gui_get_nv_texture_format(enum AVPixelFormat fmt, Uint32 *texture_fmt) {
    if (texture_fmt == NULL) {
        return 0;
    }
...
static int display_gui_get_yuv_texture_format(const AVFrame *frame, Uint32 *texture_fmt) {
...
static int display_gui_guess_colorspace(const AVFrame *frame) {
...
static app_status_t display_gui_prepare(display_gui_ctx_t *ctx, const AVFrame *frame) {
    uint8_t *planes[4] = {0};
    int linesize[4] = {0};
    ...
    ctx->use_direct_upload = display_gui_get_direct_texture_format(src_fmt, &direct_texture_fmt);
    ctx->use_nv_upload = display_gui_get_nv_texture_format(src_fmt, &nv_texture_fmt);
    ctx->use_yuv_upload = display_gui_get_yuv_texture_format(frame, &yuv_texture_fmt);
    ...
    if (ctx->use_direct_upload) {
        ...
        LOGD("gui direct upload src=%dx%d fmt=%d texture_fmt=%u",
             frame->width,
             frame->height,
             src_fmt,
             direct_texture_fmt);
    ...
    if (ctx->use_nv_upload) {
        ...
        LOGD("gui nv upload src=%dx%d fmt=%d texture_fmt=%u",
             frame->width,
             frame->height,
             src_fmt,
             nv_texture_fmt);
    ...
    if (ctx->use_yuv_upload) {
        ...
        LOGD("gui yuv upload src=%dx%d fmt=%d texture_fmt=%u",
             frame->width,
             frame->height,
             src_fmt,
             yuv_texture_fmt);
    ...
    src_cs = display_gui_guess_colorspace(frame);
    ...
    LOGD("gui prepare src=%dx%d fmt=%d -> dst=%dx%d colorspace=%d range=%d",
         frame->width,
         frame->height,
         src_fmt,
         ww,
...
app_status_t display_gui_show(display_gui_ctx_t *ctx, const video_frame_t *frame) {
    ...
    status = display_gui_prepare(ctx, frame->av_frame);
    if (status != APP_OK) {
        return status;
    }
