...
#define LOG_FILE_NAME "output_osd_rkrga.c"
...
static void output_osd_rkrga_log_error(const char *operation, int error) {
...
static void output_osd_rkrga_reset_graph(output_osd_rkrga_t *ctx) {
    if (ctx == NULL) {
        return;
    }
...
static int output_osd_rkrga_main_matches(const output_osd_rkrga_t *ctx,
...
static app_status_t output_osd_rkrga_build(output_osd_rkrga_t *ctx,
                                           const AVFrame *main_frame) {
    ...
    enum AVPixelFormat sink_formats[] = {
    ...
    output_osd_rkrga_reset_graph(ctx);
    ...
    snprintf(filter_desc,
             ...
             "repeatlast=1:shortest=0:core=rga2_core0:async_depth=0[out]",
             ctx->overlay_x,
             ctx->overlay_y);
    ...
    ret = avfilter_graph_create_filter(&ctx->main_source,
                                       ...
                                       ctx->graph);
    if (ret < 0) {
        output_osd_rkrga_log_error("create OSD main source", ret);
        goto fail;
    }
    ...
    main_params = NULL;
    if (ret < 0) {
        output_osd_rkrga_log_error("set OSD main hardware parameters", ret);
        goto fail;
    }
    ...
    ret = avfilter_graph_create_filter(&ctx->overlay_source,
                                       ...
                                       ctx->graph);
    if (ret < 0) {
        output_osd_rkrga_log_error("create OSD overlay source", ret);
        goto fail;
    }
    ret = avfilter_graph_create_filter(&ctx->sink,
                                       ...
                                       ctx->graph);
    if (ret < 0) {
        output_osd_rkrga_log_error("create OSD sink", ret);
        goto fail;
    }
    ret = av_opt_set_int_list(ctx->sink,
                              "pix_fmts",
                              sink_formats,
                              AV_PIX_FMT_NONE,
                              AV_OPT_SEARCH_CHILDREN);
    if (ret < 0) {
        output_osd_rkrga_log_error("set OSD sink format", ret);
        goto fail;
    }
    ...
    ret = avfilter_graph_parse_ptr(ctx->graph,
                                   ...
                                   NULL);
    if (ret < 0) {
        output_osd_rkrga_log_error("parse OSD RKRGA graph", ret);
        goto fail;
    }
    ...
    if (ret < 0) {
        output_osd_rkrga_log_error("configure OSD RKRGA graph", ret);
        goto fail;
    }
...
app_status_t output_osd_rkrga_init(output_osd_rkrga_t **out,
    ...
    *out = NULL;
    ctx = (output_osd_rkrga_t *)calloc(1, sizeof(*ctx));
    if (ctx == NULL) {
        return APP_ERR_NOMEM;
    }
    ctx->width = width;
...
const AVFrame *output_osd_rkrga_process(output_osd_rkrga_t *ctx,
    ...
    if (!output_osd_rkrga_main_matches(ctx, main_frame)) {
        if (output_osd_rkrga_build(ctx, main_frame) != APP_OK) {
            return NULL;
        }
    }
    ...
    if (ret < 0) {
        output_osd_rkrga_log_error("submit OSD overlay", ret);
        return NULL;
    }
    ...
    av_frame_free(&submitted);
    if (ret < 0) {
        output_osd_rkrga_log_error("submit OSD main frame", ret);
        return NULL;
    }
    ...
    if (ret < 0) {
        output_osd_rkrga_log_error("receive OSD output frame", ret);
        return NULL;
    }
...
void output_osd_rkrga_deinit(output_osd_rkrga_t **ctx_ptr) {
    output_osd_rkrga_t *ctx;
