...
#define LOG_FILE_NAME "output_encode_main.c"
...
app_status_t output_encode_main_init(output_encode_main_ctx_t *ctx,
    ...
    ctx->mode = mode;
    ctx->params = *params;
    LOGI("main encoder init mode=%d %dx%d@%d bitrate=%d",
         mode,
         params->width,
         params->height,
         params->fps,
...
app_status_t output_encode_main_push(output_encode_main_ctx_t *ctx, const video_frame_t *frame) {
    ...
    if (encode_frame == NULL) {
        LOGE("main encode prepare frame failed source=%s src=%dx%d fmt=%d",
             frame->source_name,
    ...
    LOGD("main encode frame source=%s -> %dx%d dst_fmt=NV12 converted=%d src=%dx%d fmt=%d",
         frame->source_name,
         ctx->params.width,
         ctx->params.height,
         converted,
