...
#define LOG_FILE_NAME "output_stream_rtsp.c"
...
app_status_t output_stream_rtsp_init(output_stream_rtsp_ctx_t *ctx,
    ...
    LOGI("rtsp publish opened: %s", ctx->url);
    return APP_OK;
}
...
app_status_t output_stream_rtsp_send(output_stream_rtsp_ctx_t *ctx, AVPacket *packet) {
    ...
    if (!output_stream_rtsp_is_open(ctx) || packet == NULL) {
        return APP_ERR_PARAM;
    }

    av_packet_rescale_ts(packet, ctx->input_time_base, ctx->stream->time_base);
    packet->stream_index = ctx->stream->index;

    ret = av_write_frame(ctx->format_ctx, packet);
    if (ret < 0) {
