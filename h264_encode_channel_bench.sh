...
INPUT_FILE=${1:?input NV12 file required}
WIDTH=${2:?width required}
HEIGHT=${3:?height required}
BITRATE=${4:?bitrate required}
CHANNELS=${5:?channel count required}
RUN_SECONDS=${6:-7}
LABEL=${7:-h264}
PREFIX=/tmp/h264_encode_bench_${LABEL}_${CHANNELS}
...
for ((i = 1; i <= CHANNELS; ++i)); do
    ...
    errors=$(grep -Ei 'error|failed|mpp_err|cannot|invalid' "$log_file" | \
        grep -Ev 'Error closing progress log|Estimating duration' | wc -l)
